...
def generate(admissionsFilePath, admittimeColumn, deathtimeColumn, withinDays, savePath):
        ...
        admissionsDf['MORTALITY_WITHIN_' + str(withinDays) + '_DAYS'] = pd.to_datetime(admissionsDf[deathtimeColumn], format='%Y-%m-%d %H:%M:%S.%f') < (pd.to_datetime(admissionsDf[admittimeColumn], format='%Y-%m-%d %H:%M:%S.%f') + pd.DateOffset(days=withinDays))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dtc', '--death_time_column', nargs=1, default=['deathtime'],
                        help='Name of the column containing the death time')
    ...
    log.info('args.death_time_column: ' + str(args.death_time_column[0]))
    ...
    generate(
            ...
            deathtimeColumn=args.death_time_column[0],
